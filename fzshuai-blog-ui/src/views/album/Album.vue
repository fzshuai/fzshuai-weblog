<template>
  <div>
    ...
    <v-card class="blog-container">
      <v-row>
        <v-col :md="6" v-for="item of photoAlbumList" :key="item.albumId">
          <div class="album-item">
            <v-img class="album-cover" :src="item.albumCover" />
            <router-link :to="'/albums/' + item.albumId" class="album-wrapper">
...
export default {
  ...
  methods: {
    listPhotoAlbums() {
      this.axios.get("/api/blog/photos/albums").then(({ data }) => {
